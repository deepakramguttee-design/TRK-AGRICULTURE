...
export default function Account() {
  ...
  const [editForm, setEditForm] = useState({ full_name: '', phone: '', preferred_lang: 'fr' })
  ...
  async function fetchProfile() {
    const { data } = await supabase
      .from('customers')
      .select('full_name, phone, account_type, company_name, preferred_lang')
      .eq('id', user.id)
      .single()
    if (data) {
      setProfile(data)
      setEditForm({ full_name: data.full_name ?? '', phone: data.phone ?? '', preferred_lang: data.preferred_lang ?? 'fr' })
    }
  }
